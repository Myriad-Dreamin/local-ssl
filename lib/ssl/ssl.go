...
func (env *Env) reportErr(err error) {
	env.hasErr = true
	panic(err)
}

func (env *Env) getErrBuf() error {
	if env.Err.Len() != 0 {
...
func (env *Env) validateExec() bool {
	if env.hasErr {
		return false
	}
	return true
...
func (env *Env) output() string {
	return env.Out.String()
}

func (env *Env) openSSLForced(params []string) *exec.Cmd {
...
func (env *Env) openSSL(params []string) *exec.Cmd {
	if env.hasErr {
		return nil
	}
...
func (env *Env) CheckVersion() *OpenSSLVersion {
...
func (env *Env) CheckBin() bool {
	v := env.CheckVersion()
	return v != nil
}

func (env *Env) WriteConf(confLoc string, callback func(f *os.File) error) {
	if !env.validateExec() {
		return
	}
...
func (env *Env) WriteSSLConf(confLoc string, args *SSLTemplateArgs) {
	env.WriteConf(confLoc, func(f *os.File) error {
		return RenderSSLConf(f, args)
	})
}

func (env *Env) WriteSignSSLConf(confLoc string, args *SignSSLTemplateArgs) {
	env.WriteConf(confLoc, func(f *os.File) error {
		return RenderSignSSLConf(f, args)
	})
}

func (env *Env) GenerateRSAKey(loc string) {
...
func (env *Env) GenerateRootCACrt(confLoc string, priLoc string, crtLoc string) {
...
func (env *Env) GenerateCSR(confLoc string, priLoc string, csrLoc string) {
...
func (env *Env) CreateSignedCrt(confLoc string, csrLoc string, crtLoc string, caCrtLoc string, caPriLoc string) {
...
func (env *Env) VerifyCrt(confLoc string, crtLoc string) {
...
func (env *Env) MakeDir(path string) {
	if !env.validateExec() {
		return
	}
...
func (env *Env) SetFile(path string, content string) {
